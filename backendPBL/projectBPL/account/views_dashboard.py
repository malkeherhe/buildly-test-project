    ...
    def get_enrolled_projects(self, user):
        ...
        for i, course_title in enumerate(enrolled_courses[:5]):  # آخر 5 مشاريع
            ...
            project = {
                ...
                "deadline": (timezone.now() + timedelta(days=(30 - i * 5))).strftime(
                    "%Y-%m-%d"
                ),
                "last_activity": (timezone.now() - timedelta(days=i)).strftime(
                    "%Y-%m-%d"
                ),
    ...
    def get_learning_progress(self, user):
        ...
        for i, course in enumerate(enrolled_courses[:6]):
            progress = {
                ...
                "time_spent": (i + 1) * 5,
                "last_update": (timezone.now() - timedelta(days=i)).strftime(
                    "%Y-%m-%d"
                ),
            }
    ...
    def get(self, request):
        ...
        progress_data = {
            "overall": {
                ...
                "total_hours_estimated": len(user.get_enrolled_courses_list() or [])
                * 20,
            },
            ...
            achievements.append(
                {
                    ...
                    "icon": "⭐",
                    "unlocked_at": (timezone.now() - timedelta(days=30)).strftime(
                        "%Y-%m-%d"
                    ),
                }
            )
            ...
            achievements.append(
                {
                    ...
                    "icon": "🏆",
                    "unlocked_at": (timezone.now() - timedelta(days=15)).strftime(
                        "%Y-%m-%d"
                    ),
                }
            )
