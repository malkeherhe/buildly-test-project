...
class Course(models.Model):
    ...
    description = models.TextField(
        verbose_name="وصف المسار", help_text="أدخل وصفاً مفصلاً للمسار التعليمي"
    )
    level = models.CharField(
        max_length=20, choices=LEVEL_CHOICES, default="beginner", verbose_name="المستوى"
    )
    category = models.CharField(
        max_length=20, choices=CATEGORY_CHOICES, default="other", verbose_name="الفئة"
    )
    ...
    is_public = models.BooleanField(
        default=False, verbose_name="عام للجميع", help_text="هل هذا المسار متاح للجميع؟"
    )
    ...
    is_active = models.BooleanField(
        default=True, verbose_name="نشط", help_text="هل هذا المسار نشط أم معطل؟"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT projects_count FROM courses_course WHERE id = %s", [self.pk]
                )
    ...
    def update_projects_count(self):
        ...
        try:
            ...
            actual_count = Project.objects.filter(course=self, is_active=True).count()

            # تحديث مباشر في قاعدة البيانات لتجنب recursion
            if self.projects_count != actual_count:
                Course.objects.filter(pk=self.pk).update(
                    projects_count=actual_count, updated_at=timezone.now()
                )
    ...
    def get_actual_projects_count(self):
        ...
        try:
            ...
            return Project.objects.filter(course=self, is_active=True).count()
        except:
            return 0
    ...
    def can_update_title(self, new_title):
        ...
        existing_courses = Course.objects.filter(
            title__iexact=new_title, is_active=True
        ).exclude(pk=self.pk)
