...
class CourseCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_title(self, value):
        ...
        exists = Course.objects.filter(title__iexact=value, is_active=True).exists()

        if exists:
            raise serializers.ValidationError(
                _(
                    "يوجد بالفعل مسار نشط بنفس العنوان في النظام. الرجاء اختيار عنوان مختلف."
                )
            )
    ...
    def create(self, validated_data):
        ...
        if not request.user.is_admin:
            raise serializers.ValidationError(_("يجب أن تكون مشرفاً لإنشاء مسار"))
            ...
            raise serializers.ValidationError(
                {
                    ...
                    "details": _(
                        "تم اكتشاف مسار آخر بنفس العنوان أثناء عملية الإنشاء."
                    ),
...
class CourseUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_title(self, value):
        ...
        if instance:
            ...
            exists = (
                Course.objects.filter(title__iexact=value, is_active=True)
    ...
    def update(self, instance, validated_data):
        ...
        if new_title.lower() != instance.title.lower():
            ...
            exists = (
                Course.objects.filter(title__iexact=new_title, is_active=True)
                .exclude(id=instance.id)
                .exists()
            )
...
class CourseEnrollmentSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not request.user.is_learner:
            raise serializers.ValidationError(_("يجب أن تكون متعلم للانضمام للمسار"))

        if not course.is_active:
            raise serializers.ValidationError(_("هذا المسار غير متاح حالياً"))

        if course.is_student_enrolled(request.user):
            raise serializers.ValidationError(_("أنت منضم بالفعل لهذا المسار"))
