...
class IsLearnerUser(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        return bool(
            ...
            and request.user.user_type == "learner"
        )
...
class UpdateCourseView(generics.UpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        try:
                ...
                return Response(
                    {
                        ...
                        "error": _(
                            "يوجد بالفعل مسار نشط بنفس العنوان. الرجاء اختيار عنوان مختلف."
                        ),
            ...
            return Response(
                {"success": False, "message": _("حدث خطأ غير متوقع"), "error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class DeleteCourseView(generics.DestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"success": False, "message": _("فشل حذف المسار"), "error": str(e)},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class ListCoursesView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if user.is_admin:
            ...
            return Course.objects.filter(is_active=True, is_public=True).order_by(
                "-created_at"
            )
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not queryset.exists():
            return Response({"message": _("لا توجد مسارات متاحة"), "courses": []})
                ...
                return Response(
                    {
                        ...
                        "enrollment_info": {
                            ...
                            "note": _(
                                "تم إضافتك كمتعلم في المسار وإضافة المسار إلى قائمتك"
                            ),
                        },
                    },
                    status=status.HTTP_201_CREATED,
...
class UserEnrolledCoursesView(generics.ListAPIView):
    ...
    def get_queryset(self):
        user = self.request.user
        return Course.objects.filter(enrolled_learners=user, is_active=True).order_by(
            "-created_at"
        )
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not queryset.exists():
            return Response({"message": _("لم تنضم لأي مسار بعد"), "courses": []})
...
class CheckEnrollmentView(APIView):
    ...
    def get(self, request, id):
        try:
            ...
            return Response(
                {"success": False, "message": _("المسار غير موجود")},
                status=status.HTTP_404_NOT_FOUND,
            )
