...
class Project(models.Model):
    ...
    description = models.TextField(
        verbose_name="وصف المشروع", help_text="أدخل وصفاً مفصلاً للمشروع وأهدافه"
    )
    ...
    order = models.IntegerField(
        default=0, verbose_name="الترتيب", help_text="ترتيب المشروع ضمن المسار"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.order:
            last_project = (
                Project.objects.filter(course=self.course).order_by("-order").first()
            )
