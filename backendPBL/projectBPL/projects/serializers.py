...
class ProjectCreateSerializer(serializers.ModelSerializer):
    ...
    course_id = serializers.IntegerField(
        write_only=True, required=True, help_text="معرف المسار التعليمي (course_id)"
    )
    ...
    def validate_course_id(self, value):
        ...
        if not request.user.is_admin:
            raise serializers.ValidationError(_("يجب أن تكون مشرفاً لإنشاء مشروع"))
    ...
    def validate_title(self, value):
        ...
        if len(value) < 3:
            raise serializers.ValidationError(_("العنوان يجب أن يكون على الأقل 3 أحرف"))
        ...
        if course:
            ...
            if Project.objects.filter(
                course=course, title=value, is_active=True
            ).exists():
                raise serializers.ValidationError(
                    _(
                        "يوجد بالفعل مشروع بهذا العنوان في هذا المسار. الرجاء اختيار عنوان مختلف."
                    )
                )
    ...
    def create(self, validated_data):
        ...
        try:
            # إنشاء المشروع
            project = Project.objects.create(course=course, **validated_data)
            return project
...
class ProjectUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_title(self, value):
        ...
        if (
            Project.objects.filter(course=instance.course, title=value, is_active=True)
            .exclude(id=instance.id)
            .exists()
        ):  # ⭐ تغيير: استخدم id بدلاً من project_id
            raise serializers.ValidationError(
...
class ProjectDeleteConfirmationSerializer(serializers.ModelSerializer):
    ...
    def get_deletion_impact(self, obj):
        return {
            ...
            "message": _("سيبقى {} مشروع في المسار بعد الحذف").format(
                obj.course.projects_count - 1
            ),
        }
